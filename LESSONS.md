...
## [2026-05-01] — Local environments accumulate state CI doesn't have; clean-checkout is the real dogfood

**Mistake:** Tier-1 publish-readiness work shipped with all internal gates green: `pnpm test` (34/34), `pnpm typecheck`, `pnpm lint`, `pnpm verify:tarballs` (INV-10 witness), `publint`, examples/basic vue-tsc, AND two fresh-install dogfood projects (`@effect-vue/core` and `effect-vue` meta) typechecked against packed tarballs in `/tmp`. Despite all that, the FIRST push to GitHub CI exposed TWO real failures:

1. **`@types/node` was hoisted from a transitive, not declared.** `packages/core/tsconfig.test.json` references `"types": ["node", "vitest/globals"]`. Locally pnpm's hoisting surfaced `@types/node` from a transitive dep (likely vitest or vite); CI's clean install didn't expose the same hoisting and vue-tsc errored with `TS2688: Cannot find type definition file for 'node'`. The fix was a one-liner — `pnpm add -D -w @types/node` — but the failure mode is generalizable: ANY tsconfig that references a `@types/*` package without that package being a declared (peer or dev) dep of the package can silently work locally and break elsewhere.

2. **Meta-package typecheck depends on core's `dist/`.** `packages/effect-vue/test/reexport.check.ts` imports from `"effect-vue"`, which re-exports `from "@effect-vue/core"`, whose `package.json` declares `"types": "./dist/index.d.ts"`. Locally, `dist/` always exists because prior `pnpm build` runs left it there. CI's clean checkout has no `dist/` until the build step runs — and the original CI workflow ran typecheck BEFORE build, so the meta-package's re-export was unresolvable. Fix: reorder CI to `lint → build → typecheck → test → ...`.

**Why it happened:** Both failures share the same root cause — **the local development environment accumulates state across runs (hoisted node_modules, dist/ artifacts, intermediate caches) that a clean CI environment doesn't have.** Internal verification asymptotes precisely because it runs in the same environment that authored the code. Even `/tmp/effect-vue-fresh-*/` dogfood projects (which install from a packed tarball, not workspace) couldn't catch these — they're consuming the published artifact, not the build process.

**Fix encoded in this session:**
- Declared `@types/node` explicitly in workspace devDeps (commit `dd9c78b`).
- Reordered CI to build before typecheck (commit `32bfe5e`).
- This LESSONS entry — the lesson is the rule, the rule is the encoding.

**For future agents:** when verifying a package is publish-ready, **internal gates are necessary but never sufficient**. Two clean-environment dogfoods MUST pass before claiming readiness:
1. `pnpm pack` → install in a fresh `/tmp` dir → consume the API → typecheck + run. (Catches publish-shape bugs.)
2. **A clean CI run from a clean git checkout.** (Catches local-state-accumulation bugs.)

Both are required. Either alone is incomplete. The first catches "what users see"; the second catches "what fresh contributors see." Local development is BOTH the production environment and the test environment, which makes it impossible for it to be a reliable test of itself.

**Methodology corollary for the S3 spec stack:** Add to slice done-criteria: "CI must pass on this slice's HEAD before declaring the slice ready for publish. CI must run from a clean container/runner with no preexisting `dist/`, no preexisting `node_modules`, and only declared dependencies." This is `feedback_paranoid_verification` extended one layer outward — the runtime where you run the gate must itself be untrusted.
